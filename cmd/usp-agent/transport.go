...
func NewStompTransport(cfg *config.TR369Config) *StompTransport {
	...
	sendDest := cfg.STOMPDestinationOutbound // agent → controller (outbound from agent perspective)
	subDest := cfg.STOMPDestinationInbound   // controller → agent (inbound to agent perspective)
...
func (s *StompTransport) Connect() error {
	...
	log.Printf("🚀 STOMP: Starting readLoop goroutine...")
	go s.readLoop()
	if s.hbSend > 0 {
		log.Printf("💓 STOMP: Starting heartbeat goroutine...")
		go s.heartbeatLoop()
	}
	log.Printf("✅ STOMP: All goroutines started")
	return nil
}
...
func (s *StompTransport) readLoop() {
	log.Printf("========================================")
	log.Printf("🚀🚀🚀 STOMP readLoop STARTED 🚀🚀🚀")
	log.Printf("========================================")
	reader := bufio.NewReader(s.conn)
	for {
		s.conn.SetReadDeadline(time.Now().Add(30 * time.Second))
		log.Printf("📖 STOMP readLoop: About to read frame...")
		frame, err := readStompFrame(reader)
		if err != nil {
			log.Printf("🔍 STOMP readLoop error: %v", err)
			select {
			case s.errs <- err:
			default:
			}
		...
		if frame.command == "MESSAGE" {
			log.Printf("📨 STOMP readLoop: Received MESSAGE frame, body size: %d bytes", len(frame.body))
			s.inbox <- frame.body
			log.Printf("✅ STOMP readLoop: MESSAGE added to inbox")
		} else if frame.command == "HEARTBEAT" {
			log.Printf("💓 STOMP readLoop: Received heartbeat")
		} else {
			log.Printf("🔍 STOMP readLoop: Received frame command: %s", frame.command)
		}
	}
}
...
func (s *StompTransport) Read(timeout time.Duration) ([]byte, error) {
	log.Printf("🔍 STOMP Read: Waiting for message (timeout: %s)...", timeout)
	select {
	case msg := <-s.inbox:
		log.Printf("✅ STOMP Read: Received message from inbox (%d bytes)", len(msg))
		return msg, nil
	case err := <-s.errs:
		log.Printf("❌ STOMP Read: Received error from errs channel: %v", err)
		return nil, err
	case <-time.After(timeout):
		log.Printf("⏰ STOMP Read: Timeout after %s", timeout)
